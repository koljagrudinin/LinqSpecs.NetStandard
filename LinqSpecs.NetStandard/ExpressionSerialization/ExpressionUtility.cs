...
namespace LinqSpecs.NetStandard.ExpressionSerialization
{
    /// <summary>
    /// ExpressionUtility
    /// </summary>
    static class ExpressionUtility
    {
        ...
        private static Expression EnsureUnary(UnaryExpression expression)
        {
            ...
            return operand != expression.Operand
                ? Expression.MakeUnary(expression.NodeType, operand, expression.Type, expression.Method)
                : expression;
        }
        ...
        private static Expression EnsureLambda(LambdaExpression expression)
        {
            ...
            return body != expression.Body
                ? Expression.Lambda(expression.Type, body, expression.Parameters)
                : expression;
        }
        ...
        private static NewExpression EnsureNew(NewExpression expression)
        {
            ...
            if (args != expression.Arguments)
                return expression.Members != null
                    ? Expression.New(expression.Constructor, args, expression.Members)
                    : Expression.New(expression.Constructor, args);
        ...
        private static Expression EnsureNewArray(NewArrayExpression expression)
        {
            ...
            if (exprs != expression.Expressions)
                return expression.NodeType == ExpressionType.NewArrayInit
                    ? Expression.NewArrayInit(expression.Type.GetElementType(), exprs)
                    : Expression.NewArrayBounds(expression.Type.GetElementType(), exprs);
